...
function setup() {
  ...
  menu = new Menu();//instancia um objeto para o menu inicial
...
function draw() {
    ...
    }else if(keyIsDown(UP_ARROW) || player.jumpTest()){//TESTA SE O MOVIMENTO É UM PULO OU SE ESTÁ CAINDO
      player.jump();
      mov = true;
    }
    //MOVIMENTAÇÃO PRA ESQUEDA E DIREITA, SENSAÇÃODE MOVIMENTAÇÃO VEM DO BG
    //A FUNÇÃO keyIsDown É DA BIBLIOTECA P5JS E IDENTIFICA SE UMA TECLA ESTÁ PRESSIONADA
    if(keyIsDown(LEFT_ARROW)){
      dir = (1);
      Bg.setPosition(dir);//DEFINE A POSIÇÃO DO MOVIMENTO COM RELAÇÃO A DIREÇÃO
    ...
    //INDENTIFICADOR DE HIT DO HALTER / AS CONDIÇÕES DO IF SÃO PARA DELIMITAR A ÁREA DO HALTER, NA QUAL O JOGADOR NÃO PODE PASSAR
    ...
    //MOSTRA A PONTUAÇÃO
    player.scoreShow(burguer.getDamageFlag());
    //DANOS DO CHEFE
